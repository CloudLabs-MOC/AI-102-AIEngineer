...
In this task, you create a new project in Microsoft Foundry, deploy the gpt-4.1 model, and save it as an agent for further configuration.
...
1. In the navigation pane, select **Agents (1)**, and then choose **Create agent (2)**.

    ![](../Images/lab17-03-2.png)

1. In the **Create an agent** pane, enter **product-expert-agent (1)** in the **Agent name** field, and then select **Create (2)**.

    ![](../Images/lab17-03-3.png)
...
In this task, you will configure the agent in Microsoft Foundry to use Foundry IQ by connecting it to an Azure AI Search resource. You also create a knowledge base using product documents stored in Azure Blob Storage to enable accurate and grounded responses.
...
1. When the application starts, test the agent with the following queries:
    ...
    **Query 1 - Product Categories:**
    ...
    ![](../Images/lab17-03-9.png)

1. When prompted for approval, type **yes** to allow the agent to search the knowledge base.

    ![](../Images/lab17-03-10.png)

1. Observe how the agent retrieves information from multiple documents in the knowledge base.

    ![](../Images/lab17-03-11.png)

1. Try the following test queries and review the responses.
